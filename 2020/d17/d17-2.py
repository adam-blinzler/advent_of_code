def neighbor_offset():
    ...
    for dx in range(-1, 2):
        for dy in range(-1, 2):
            for dz in range(-1, 2):
                for dw in range (-1, 2):
                        rng.append([dx,dy,dz,dw])
    return rng

def how_many_neighbors(active,point):
    total = 0
    for x,y,z,w in neighbor_offset():
       if not ( x == 0 and y == 0 and z == 0 ):
            if [point[0]+x,point[1]+y,point[2]+z,point[3]+w] in active:
                total += 1
    return total
...
for _ in range(rounds):
    ...
    for point in active:
        for x,y,z,w in neighbor_offset():
            sus_point = [point[0]+x,point[1]+y,point[2]+z,point[3]+w]
